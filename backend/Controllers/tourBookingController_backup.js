...
const generateTourBookingInvoice = async (req, res) => {
  try {
    ...
    doc.fillColor('#235784')
       .fontSize(12)
       .font('Helvetica-Bold')
       .text(`Base Price: LKR ${booking.pricing.basePrice.toLocaleString()}`, 60, 430);
    
    if (booking.pricing.discountApplied > 0) {
      doc.text(`Discount: -LKR ${booking.pricing.discountApplied.toLocaleString()}`, 60, 450);
    }
    ...
    doc.fillColor('#235784')
       .fontSize(16)
       .font('Helvetica-Bold')
       .text(`Final Price: LKR ${booking.pricing.finalPrice.toLocaleString()}`, 420, 435);
